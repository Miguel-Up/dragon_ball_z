...
require("dotenv").config() //accedemos a toda los archivos en .env
...
mongoose.connect(process.env.DB_URL)
    ...
    }).catch((err) => { console.log('error al borrar') })
    .then(async () => {
        const charactersDocs = arrayCharacters.map((eachCharacter) => new Characters(eachCharacter))
        await Characters.insertMany(charactersDocs)

    })
    .catch((err) => { console.log('error al insera') })
